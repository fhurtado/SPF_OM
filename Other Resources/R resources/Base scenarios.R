## Australian SPF simulation code.
...
if(run.again==T){
  ...
  for(stock in 1:length(Stocks)){
    ...
    # Loop over Fs
    for(ff in 1:length(scenarios)){
      # Create the folders needed
      Fpath <- paste0(stockpath,"/Sce_",ff)
      dir.create(Fpath)
      setwd(Fpath)
      ...
      if(ff==1) t.specfile[20] <- "1"
      else {
        t.specfile[20] <- "0"
        t.specfile[6] <- paste(FFs[ff-1,stock],0.7,sep="\t")
      } 
...
if(read.again.R==T){
  ...
  write.csv(plot.table,"final.csv", row.names=F)
}
...
if(read.again.for==T){
  ...
  for(stock in 1:length(Stocks)){
    ...
    for(ff in 1:length(FFs)){
      ...
      #Save to final table
      plot.table <- cbind(plot.table,c(outs[,2],outs[,5],
                                         outs[,3],outs[,6],
                                         outs[,4],outs[,7]))
      cat(Stocks[stock],"F =",FFs[ff],"\n")
    }
